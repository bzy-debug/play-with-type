...
let rec unify constraints =
  ...
  | (TArr(t1, t2), TArr(t3, t4))::rest ->
      ...
      print_endline (string_of_typ_typ_list rest);
      unify rest
...
let infer exp =
  ...
  print_endline (string_of_typ aim);
...
(* let s x y z = (x z) (y z) *)
(* ('a -> 'b -> 'c) -> ('a -> 'b) -> 'a -> 'c *)
let s = Fun("x", Fun("y", Fun("z", App(App(Name "x", Name "z"), App(Name "x", Name "z")))))

let tests = [test; id; compose; first; add; succ; le; le10; s]

let () =
  s |> infer |> string_of_typ |> print_endline
